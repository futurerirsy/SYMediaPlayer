...
#import "SYPlayerDistortionModel.h"

#define SY_GLES_STRINGIZE(x)    #x

static const char vertexShaderString[] = SY_GLES_STRINGIZE
(
...
static const char fragmentShaderString[] = SY_GLES_STRINGIZE
(
...
@interface SYPlayerDistortionRenderer ()
{
...
@property (nonatomic, strong) SYPlayerDistortionModel * leftEye;
@property (nonatomic, strong) SYPlayerDistortionModel * rightEye;

@end


@implementation SYPlayerDistortionRenderer
...
- (void)afterDrawFrame
{
    glViewport(0, 0, (GLsizei)self.viewportSize.width, (GLsizei)self.viewportSize.height);
    ...
    glScissor(0, 0, (GLsizei)(self.viewportSize.width / 2), (GLsizei)self.viewportSize.height);
    [self draw:self.leftEye];
    glScissor((GLint)self.viewportSize.width / 2, 0,
              (GLsizei)(self.viewportSize.width / 2), (GLsizei)self.viewportSize.height);
    [self draw:self.rightEye];
    
    glDisable(GL_SCISSOR_TEST);
}

- (SYPlayerDistortionModel *)leftEye
{
    if (!_leftEye) {
        _leftEye = [SYPlayerDistortionModel modelWithModelType:SYPlayerDistortionModelTypeLeft];
    }
    return _leftEye;
}

- (SYPlayerDistortionModel *)rightEye
{
    if (!_rightEye) {
        _rightEye = [SYPlayerDistortionModel modelWithModelType:SYPlayerDistortionModelTypeRight];
    }
    return _rightEye;
}

- (void)draw:(SYPlayerDistortionModel *)eye
{
    [self useProgram];
...
- (void)resetFrameBufferSize
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB,
                 (GLsizei)self.viewportSize.width, (GLsizei)self.viewportSize.height,
