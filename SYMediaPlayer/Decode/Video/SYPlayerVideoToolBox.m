...
typedef NS_ENUM(NSUInteger, SYPlayerVideoToolBoxErrorCode) {
    SYPlayerVideoToolBoxErrorCodeExtradataSize,
    SYPlayerVideoToolBoxErrorCodeExtradataData,
    SYPlayerVideoToolBoxErrorCodeCreateFormatDescription,
    SYPlayerVideoToolBoxErrorCodeCreateSession,
    SYPlayerVideoToolBoxErrorCodeNotH264,
};


@interface SYPlayerVideoToolBox ()
{
...
@implementation SYPlayerVideoToolBox
...
- (NSError *)setupVTSession
{
    ...
    if (codec_id == AV_CODEC_ID_H264) {
        if (extradata_size < 7 || extradata == NULL) {
            error = [NSError errorWithDomain:@"extradata error"
                                        code:SYPlayerVideoToolBoxErrorCodeExtradataSize userInfo:nil];
            return error;
        }
        ...
        if (extradata[0] == 1) {
            ...
            self->_format_description = CreateFormatDescription(kCMVideoCodecType_H264,
                                                                _codec_context->width,
                                                                _codec_context->height,
                                                                extradata, extradata_size);
            if (self->_format_description == NULL) {
                error = [NSError errorWithDomain:@"create format description error"
                                            code:SYPlayerVideoToolBoxErrorCodeCreateFormatDescription userInfo:nil];
                return error;
            }
            
            CFMutableDictionaryRef destinationPixelBufferAttributes =
            CFDictionaryCreateMutable(NULL, 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
            cf_dict_set_int32(destinationPixelBufferAttributes,
            ...
            OSStatus status = VTDecompressionSessionCreate(kCFAllocatorDefault,
                                                           ...
                                                           &self->_vt_session);
            if (status != noErr) {
                error = [NSError errorWithDomain:@"create session error"
                                            code:SYPlayerVideoToolBoxErrorCodeCreateSession userInfo:nil];
                return error;
            }
            ...
            error = [NSError errorWithDomain:@"deal extradata error"
                                        code:SYPlayerVideoToolBoxErrorCodeExtradataData userInfo:nil];
            return error;
        }
    }
    else {
        error = [NSError errorWithDomain:@"not h264 error"
                                    code:SYPlayerVideoToolBoxErrorCodeNotH264 userInfo:nil];
        return error;
    }
...
static void outputCallback(void * decompressionOutputRefCon, void * sourceFrameRefCon,
                           OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef imageBuffer,
                           CMTime presentationTimeStamp, CMTime presentationDuration)
{
    @autoreleasepool
    {
        SYPlayerVideoToolBox * videoToolBox = (__bridge SYPlayerVideoToolBox *)decompressionOutputRefCon;
...
static CMFormatDescriptionRef CreateFormatDescription(CMVideoCodecType codec_type, int width, int height,
